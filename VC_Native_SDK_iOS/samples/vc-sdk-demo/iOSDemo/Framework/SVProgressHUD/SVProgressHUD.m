...
- (void)moveToPoint:(CGPoint)newCenter rotateAngle:(CGFloat)angle {
    ...
    if (self.containerView) {
        ...
        if (self.containerView.center.y <1) {
                  y  = [UIScreen mainScreen].bounds.size.height/2.0;
               }
        self.hudView.center = CGPointMake(self.containerView.center.x + self.offsetFromCenter.horizontal, y + self.offsetFromCenter.vertical);
    } else {
        if (newCenter.y <1) {
            newCenter.y = [UIScreen mainScreen].bounds.size.height/2.0;
        }
