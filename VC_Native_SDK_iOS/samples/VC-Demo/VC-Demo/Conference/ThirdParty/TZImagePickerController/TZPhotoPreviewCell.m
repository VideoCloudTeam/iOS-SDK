...
- (void)resizeSubviews {
    ...
    _imageContainerView.tz_height = self.scrollView.tz_height;
    
    UIImage *image = _imageView.image;
    if (image.size.height / image.size.width > self.tz_width / self.scrollView.tz_height) {
        _imageContainerView.tz_width = floor(image.size.width / (image.size.height / self.scrollView.tz_height));
    } else {
        CGFloat height = image.size.width / image.size.height * self.scrollView.tz_height;
        if (height < 1 || isnan(height)) height = self.tz_width;
        height = floor(height);
        _imageContainerView.tz_width = height;
        _imageContainerView.tz_centerX = self.tz_width / 2;
    }
    if (_imageContainerView.tz_width > self.tz_width && _imageContainerView.tz_width - self.tz_height <= 1) {
        _imageContainerView.tz_width = self.tz_width;
    }
    CGFloat contentSizeH = MAX(_imageContainerView.tz_width, self.tz_width);
    _scrollView.contentSize = CGSizeMake(contentSizeH,self.scrollView.tz_height);
    [_scrollView scrollRectToVisible:self.bounds animated:NO];
    _scrollView.alwaysBounceVertical = _imageContainerView.tz_width <= self.tz_width ? NO : YES;
