...
- (UIImage *)narrowImage:(UIImage *)imange {
    CIImage *ciimage = [CIImage imageWithCGImage:imange.CGImage];
    CIContext *temporaryContext = [CIContext contextWithOptions:nil];
    CIFilter *filter = [CIFilter filterWithName:@"CIAffineTransform"];
    [filter setValue:ciimage forKey:@"inputImage"];
    
//    CGFloat sc = imange.size.height >= SCREEN_HEIGHT ?  SCREEN_HEIGHT / imange.size.height :  imange.size.height;
    
    [filter setValue: [NSValue valueWithCGAffineTransform:CGAffineTransformMakeScale(1,1)] forKey:@"inputTransform"];
    CIImage *outputImage = filter.outputImage;
    CGImageRef videoImage = [temporaryContext createCGImage:outputImage fromRect:CGRectMake(0, 0,imange.size.width,
                                                                                            imange.size.height)];
    UIImage *image1 = [[UIImage alloc] initWithCGImage:videoImage];
